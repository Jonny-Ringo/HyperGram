...
<body>
    ...
    <script>
        ...
        async function createNewChat(otherUser) {
            try {
                // Create new chat process using proper HyperBeam endpoint format
                const participants = `${userAddress},${otherUser}`;
                const url = `/${processId}/push?action=create-chat&participants=${encodeURIComponent(participants)}`;
...
-- Message storage handler - Following HyperSite add-comment pattern
Handlers.add('SendMessage', 'send-message', function(msg)
    print("🔧 MSG: Handler started")
    
    local chat_id = msg.chat_id or msg["chat_id"] or msg.Tags.chat_id or msg.Tags["chat_id"]
    local message_content = msg.message or msg.Tags.message or msg.Data or msg["message"]
    local sender = msg.sender  or msg.Tags.sender
    
    print("🔧 MSG: Chat ID: " .. tostring(chat_id))
    print("🔧 MSG: Message: " .. tostring(message_content))
    print("🔧 MSG: Sender: " .. tostring(sender))
    
    -- Skip all validation for now, just store the message
    -- Find the chat by searching for partial match since we don't know the full ID
    ...
    -- If no chat found, just use the first available chat for testing
    ...
    -- Just store the message if we have a target chat
    if target_chat and message_content then
        ...
        send({
            target = msg.From,
            data = json.encode({status = "success", message = "Message stored"})
        })
    else
        send({
            target = msg.From,
            data = json.encode({status = "error", message = "No chat or message found"})
        })
    end
end)
